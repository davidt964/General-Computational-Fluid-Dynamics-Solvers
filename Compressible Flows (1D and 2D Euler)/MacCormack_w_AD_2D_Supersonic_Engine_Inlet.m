...
% AUTHOR: David L. Tran
%
% MAE 250D Final Project (MacCormack scheme with artificial dissipation
% method ONLY for this script)
%
% DESCRIPTION: Solves the MacCormack predictor-corrector scheme with
% additional artificial dissipation for 2D.
%
%==========================================================================
...
epsilon = 0.6; %Artificial dissipation parameter
...
while (count < IT_MAX) && (err > tol)
   ...
   for j=2:JL-1
       for i=2:IL-1
           ...
           EprimeE=Fluxprime(e1(i+1,j),e2(i+1,j),e3(i+1,j),e4(i+1,j)...
                ,f1(i+1,j),f2(i+1,j),f3(i+1,j),f4(i+1,j),nxE(i,j),nyE(i,j));
            EprimeW=Fluxprime(e1(i,j),e2(i,j),e3(i,j),e4(i,j)...
                ,f1(i,j),f2(i,j),f3(i,j),f4(i,j),nxW(i,j),nyW(i,j));
            FprimeN=Fluxprime(e1(i,j+1),e2(i,j+1),e3(i,j+1),e4(i,j+1)...
                ,f1(i,j+1),f2(i,j+1),f3(i,j+1),f4(i,j+1),nxN(i,j),nyN(i,j));
            FprimeS=Fluxprime(e1(i,j),e2(i,j),e3(i,j),e4(i,j)...
                ,f1(i,j),f2(i,j),f3(i,j),f4(i,j),nxS(i,j),nyS(i,j));
          ...
          if (i == 2) && (j == 2)
              ...
              %use the pressure extrapolation terms
              P=[p(i,j);p(i+1,j);p(i-1,j);2*p(1,j)-p(2,j);p(i,j+1);...
                p(i,j-1);2*p(i,1)-p(i,2)];
          ...
          elseif (j == 2) && (i ~= 2)
              %Bottom wall
              P=[p(i,j);p(i+1,j);p(i-1,j);p(i-2,j);p(i,j+1);...
                p(i,j-1);2*p(i,1)-p(i,2)];
            ...
            %Calculate predictor
            Ubar = Mac_PredAD(epsilon,dt1(count),V(i,j),S,Nor,...
                U,UIp,UIm,UJp,UJm,uVel,vVel,C,...
                P,EprimeE,EprimeW,FprimeN,FprimeS);

            %Store values of predictor
            ubar1(i,j)=Ubar(1);
            ubar2(i,j)=Ubar(2);
            ubar3(i,j)=Ubar(3);
            ubar4(i,j)=Ubar(4);
   ...
   %%      Boundary Condition     %%
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   pbar = pressure(gamma,ubar1,ubar2,ubar3,ubar4); 
    ubar=u_vel(ubar1,ubar2);                 %x-velocity
    vbar=v_vel(ubar1,ubar3);                 %y-velocity

   [ubar1,ubar2,ubar3,ubar4,pbar,ubar,vbar]=...
       boundarycondition(IL,JL,IS,t1,...
       gamma,ubar1,ubar,vbar,pbar);

    cbar=sos(gamma,pbar,ubar1);              %Speed of sound
    h = (ubar4 + pbar) ./ ubar1;               %enthalpy
  
  
   [ebar1,ebar2,ebar3,ebar4]=...
       Eflux(gamma,ubar1,ubar2,ubar3,ubar4);   %Flux vectors in conservative variables
   [fbar1,fbar2,fbar3,fbar4]=...
       Fflux(gamma,ubar1,ubar2,ubar3,ubar4);   %Flux vectors in conservative variables
 
                      %%Corrector Step
    for j = 2:JL-1
        for i = 2:IL-1
            EprimeE=Fluxprime(ebar1(i,j),ebar2(i,j),ebar3(i,j),ebar4(i,j)...
                ,fbar1(i,j),fbar2(i,j),fbar3(i,j),fbar4(i,j),nxE(i,j),nyE(i,j));
            EprimeW=Fluxprime(ebar1(i-1,j),ebar2(i-1,j),ebar3(i-1,j),ebar4(i-1,j)...
                ,fbar1(i-1,j),fbar2(i-1,j),fbar3(i-1,j),fbar4(i-1,j),nxW(i,j),nyW(i,j));
            FprimeN=Fluxprime(ebar1(i,j),ebar2(i,j),ebar3(i,j),ebar4(i,j)...
                ,fbar1(i,j),fbar2(i,j),fbar3(i,j),fbar4(i,j),nxN(i,j),nyN(i,j));
            FprimeS=Fluxprime(ebar1(i,j-1),ebar2(i,j-1),ebar3(i,j-1),ebar4(i,j-1)...
                ,fbar1(i,j-1),fbar2(i,j-1),fbar3(i,j-1),fbar4(i,j-1),nxS(i,j),nyS(i,j));
            U=[u1(i,j);u2(i,j);u3(i,j);u4(i,j)];
            Ubar1=[ubar1(i,j);ubar2(i,j);ubar3(i,j);ubar4(i,j)];
            UIp=[ubar1(i+1,j);ubar2(i+1,j);ubar3(i+1,j);ubar4(i+1,j)];
            UIm=[ubar1(i-1,j);ubar2(i-1,j);ubar3(i-1,j);ubar4(i-1,j)];
            UJp=[ubar1(i,j+1);ubar2(i,j+1);ubar3(i,j+1);ubar4(i,j+1)];
            UJm=[ubar1(i,j-1);ubar2(i,j-1);ubar3(i,j-1);ubar4(i,j-1)];
            S=[sE(i,j);sW(i,j);sN(i,j);sS(i,j)];
            uVel=[ubar(i,j);ubar(i-1,j);ubar(i,j-1)];
            vVel=[vbar(i,j);vbar(i-1,j);vbar(i,j-1)];
            C=[cbar(i,j);cbar(i-1,j);cbar(i,j-1)];

           if (i == 2) && (j == 2)
              %Bottom left corner

              %use the pressure extrapolation terms
              P=[pbar(i,j);pbar(i+1,j);pbar(i-1,j);2*pbar(1,j)-pbar(2,j);pbar(i,j+1);...
                pbar(i,j-1);2*pbar(i,1)-pbar(i,2)];

          elseif (i == 2) && (j ~= 2)
              %Left wall
              P=[pbar(i,j);pbar(i+1,j);pbar(i-1,j);2*pbar(1,j)-pbar(2,j);pbar(i,j+1);...
                pbar(i,j-1);pbar(i,j-2)];

          elseif (j == 2) && (i ~= 2)
              %Bottom wall
              
              P=[pbar(i,j);pbar(i+1,j);pbar(i-1,j);pbar(i-2,j);pbar(i,j+1);...
                pbar(i,j-1);2*pbar(i,1)-pbar(i,2)];
          ...
          %Calculate corrector step
            Unew=Mac_CorrAD(epsilon,dt1(count),V(i,j),S,Nor,...
                U,Ubar1,UIp,UIm,UJp,UJm,uVel,vVel,C,...
                P,EprimeE,EprimeW,FprimeN,FprimeS);
   ...
   u4=unew4;
    ...
    p=pressure(gamma,u1,u2,u3,u4);                  %calculate new pressure   
    u=u_vel(u1,u2);                 %x-velocity
    v=v_vel(u1,u3);                 %y-velocity
...
title(['MacCormack Scheme with Artificial Dissipation ($\epsilon$ = ' num2str(epsilon) ')'], 'Interpreter','LaTeX');
...
%                                                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function D1=art_visc(epsilon,u,c,Pp,P,Pm,Up,U)

D1=-epsilon*(abs(u)+c)*(abs(Pp-(2*P)+Pm)/(Pp+(2*P)+Pm))*(Up-U); 

end

function Ubar=Mac_PredAD(epsilon,dt,V,S,Nor,...
    U,UIp,UIm,UJp,UJm,uVel,vVel,C,...
    P,EprimeE,EprimeW,EprimeN,EprimeS)
...
DIp=art_visc(epsilon,VE,cIp,pIp,p,pIm,UIp,U);   %Artificial viscosity
DI=art_visc(epsilon,VW,c,p,pIm,pImm,U,UIm);       %Artificial viscosity
DJp=art_visc(epsilon,VN,cJp,pJp,p,pJm,UJp,U);
DJ=art_visc(epsilon,VS,c,p,pJm,pJmm,U,UJm);
...
Ubar=U-(dt/V)*(F1*s1-F2*s2+F3*s3-F4*s4);


end

function Unew=Mac_CorrAD(epsilon,dt,V,S,Nor,...
    U,Ubar,UIp,UIm,UJp,UJm,uVel,vVel,C,...
    P,EprimeE,EprimeW,EprimeN,EprimeS)
...
DIp=art_visc(epsilon,VE,c,pIp,p,pIm,UIp,U);   %Artificial viscosity
DI=art_visc(epsilon,VW,cIm,p,pIm,pImm,U,UIm);       %Artificial viscosity
DJp=art_visc(epsilon,VN,c,pJp,p,pJm,UJp,U);
DJ=art_visc(epsilon,VS,cJm,p,pJm,pJmm,U,UJm);
...
Unew=0.5*(U+Ubar-(dt/V)*(F1*s1-F2*s2+F3*s3-F4*s4));
