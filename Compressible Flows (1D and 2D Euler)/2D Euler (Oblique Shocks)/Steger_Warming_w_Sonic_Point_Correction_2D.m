...
% AUTHOR: David L. Tran
%
% Steger-Warming method with sonic-point correction ONLY for this script
%
% DESCRIPTION: Solves the Steger-Warming scheme with a sonic-point 
% correctionfor a 2D supersonic engine inlet with prescribed supersonic 
% inlet conditions. A grid is generated using the differential equation
% method which uses an algebraically generated grid as an initial guess. 
% The numerical approximation is repeated until convergence within a set 
...
while (count < IT_MAX) && (err > tol)
   ...
   for j=2:JL-1
       for i=2:IL-1         
           %Calculate A-/A+ for every term (depending on the term)
           A_ip1_minus = calcAminus(u(i+1,j), v(i+1,j), rho(i+1,j), c(i+1,j), nxE(i,j), nyE(i,j), gamma);
           A_i_plus = calcAplus(u(i,j), v(i,j), rho(i,j), c(i,j), nxE(i,j), nyE(i,j), gamma); %with E_{i+1,j}^{''-}
           A_i_minus = calcAminus(u(i,j), v(i,j), rho(i,j), c(i,j), nxW(i,j), nyW(i,j), gamma); %with E_{i-1,j}^{''+}
           A_im1_plus = calcAplus(u(i-1,j), v(i-1,j), rho(i-1,j), c(i-1,j), nxW(i,j), nyW(i,j), gamma);
           A_jp1_minus = calcAminus(u(i,j+1), v(i,j+1), rho(i,j+1), c(i,j+1), nxN(i,j), nyN(i,j), gamma);
           A_j_plus = calcAplus(u(i,j), v(i,j), rho(i,j), c(i,j), nxN(i,j), nyN(i,j), gamma); %with F_{i,j+1}^{''-}
           A_j_minus = calcAminus(u(i,j), v(i,j), rho(i,j), c(i,j), nxS(i,j), nyS(i,j), gamma); %with F_{i,j-1}^{''+}
           A_jm1_plus = calcAplus(u(i,j-1), v(i,j-1), rho(i,j-1), c(i,j-1), nxS(i,j), nyS(i,j), gamma);         
           ...
           E_ip1_minus = A_ip1_minus * UIp;
           E_i_plus = A_i_plus * U; %with E_{i+1,j}^{''-}
           E_i_minus = A_i_minus * U; %with E_{i-1,j}^{''+}
           E_im1_plus = A_im1_plus * UIm;
           F_jp1_minus = A_jp1_minus * UJp;
           F_j_plus = A_j_plus * U; %with F_{i,j+1}^{''-}
           F_j_minus = A_j_minus * U; %with F_{i,j-1}^{''+}
           F_jm1_plus = A_jm1_plus * UJm;
           ...
           %Update the step
           [U] = SW(dt1(count), V(i,j), S, U, E_ip1_minus, E_i_plus, E_i_minus, E_im1_plus,...
                   F_jp1_minus, F_j_plus, F_j_minus, F_jm1_plus);
...
title('Steger-Warming Flux-Vector Splitting w/ Sonic-Point Correction Scheme','Interpreter','LaTeX');
...
function [newFlux] = SWFlux(Flux_i, Flux_ip1, U_i, U_ip1, A)
%This function calculates the Steger-Warming flux.
newFlux = 0.5 * (Flux_i + Flux_ip1) - 0.5 * A * (U_ip1 - U_i);
end

function [A_plus] = calcAplus(u, v, rho, c, n_x, n_y, gamma)
%Calculates A+ for the Steger-Warming scheme.
...
%populate Lambda^{+} using the (i or j) state (1 index value less than A_minus)
Lambda_p(1,1) = (u_prime + abs(u_prime)) / 2;
Lambda_p(2,2) = ( (u_prime + c) + abs(u_prime + c) ) / 2;
Lambda_p(3,3) = (u_prime + abs(u_prime)) / 2;
Lambda_p(4,4) = ( (u_prime - c) + abs(u_prime - c) ) / 2;

if abs(Lambda_p(1,1)) < 2 * eps_corr
    %Perform correction for 1st and 3rd eigenvalues (identical)
    Lambda_p(1,1) = (Lambda_p(1,1)^2 + 4 * eps_corr^2) / (4 * eps_corr);
    Lambda_p(3,3) = (Lambda_p(3,3)^2 + 4 * eps_corr^2) / (4 * eps_corr);

elseif abs(Lambda_p(2,2)) < 2 * eps_corr
    %Perform correction for 2nd eigenvalue
    Lambda_p(2,2) = (Lambda_p(2,2)^2 + 4 * eps_corr^2) / (4 * eps_corr);    
       
elseif abs(Lambda_p(4,4)) < 2 * eps_corr
    %Perform correction for 4th eigenvalue
    Lambda_p(4,4) = (Lambda_p(4,4)^2 + 4 * eps_corr^2) / (4 * eps_corr);

end 

%Calculate A_plus for the (i or j) state (1 index value less than A_minus)
A_plus = P * Lambda_p * P_inv;
end

function [A_minus] = calcAminus(u, v, rho, c, n_x, n_y, gamma)
%Calculates A- for the Steger-Warming scheme.
...
%populate Lambda^{-}
Lambda_m(1,1) = (u_prime - abs(u_prime)) / 2;
Lambda_m(2,2) = ( (u_prime + c) - abs(u_prime + c) ) / 2;
Lambda_m(3,3) = (u_prime - abs(u_prime)) / 2;
Lambda_m(4,4) = ( (u_prime - c) - abs(u_prime - c) ) / 2;

if abs(Lambda_m(1,1)) < 2 * eps_corr
    %Perform correction for 1st and 3rd eigenvalues (identical)
    Lambda_m(1,1) = (Lambda_m(1,1)^2 + 4 * eps_corr^2) / (4 * eps_corr);
    Lambda_m(3,3) = (Lambda_m(3,3)^2 + 4 * eps_corr^2) / (4 * eps_corr);

elseif abs(Lambda_m(2,2)) < 2 * eps_corr
    %Perform correction for 2nd eigenvalue
    Lambda_m(2,2) = (Lambda_m(2,2)^2 + 4 * eps_corr^2) / (4 * eps_corr);    
       
elseif abs(Lambda_m(4,4)) < 2 * eps_corr
    %Perform correction for 4th eigenvalue
    Lambda_m(4,4) = (Lambda_m(4,4)^2 + 4 * eps_corr^2) / (4 * eps_corr);

end 

%Calculate A_minus for the (i or j) + 1 state
A_minus = P * Lambda_m * P_inv;

end

function [U_new] = SW(dt, V, S, U, E_ip1_minus, E_i_plus, E_i_minus, E_im1_plus,...
   F_jp1_minus, F_j_plus, F_j_minus, F_jm1_plus)
...
U_new = U - dt / V .* ( ( (E_ip1_minus + E_i_plus).*s1 - (E_i_minus + E_im1_plus).*s2  ) ...
                   + ( (F_jp1_minus + F_j_plus).*s3 - (F_j_minus + F_jm1_plus).*s4 ) );
end
