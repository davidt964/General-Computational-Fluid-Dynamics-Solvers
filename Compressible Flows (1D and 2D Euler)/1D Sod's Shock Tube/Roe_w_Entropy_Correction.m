...
% AUTHOR: David L. Tran
%
% Roe scheme with entropy correction
%
% DESCRIPTION: Numerically solves the conservative 1D Euler equation in a
% finite volume formulation using the Roe scheme with entropy correction. 
% Initial conditions correspond to those of Sod's shock tube. This is a 
% Riemann problem wherein sharp discontinuities are present in the exact 
% solution.
%
%==========================================================================
...
%Roe's Method with entropy correction for Grid 1
for k = 1:length(nu)
    %Initialize IC
    ...
    for n = 1:t_steps(1)
        ...
        for i = 2:IL(1)-1
            ...
            %Perform entropy correction for cases when lambda_{i+1/2} and
            %lambda_{i-1/2} are < 2*epsilon
            if abs(u_avg) < 2*epsilon
                %check first eigenvalue for lambda_{i+1/2}
                Lambda_i_half = [(abs(u_avg)^2 + 4*epsilon^2) / (4*epsilon),    0,               0;...
                                    0,                          abs(u_avg + c_avg),              0;...
                                    0,                          0,                   abs(u_avg - c_avg)];
%                 fprintf('1');

            elseif abs(u_avg + c_avg) < 2*epsilon
                %check second eigenvalue for lambda_{i+1/2}
                Lambda_i_half = [abs(u_avg),                 0,               0;...
                                    0,         (abs(u_avg + c_avg)^2 + 4*epsilon^2) / (4*epsilon),              0;...
                                    0,                          0,                   abs(u_avg - c_avg)];
%                 fprintf('2');

            elseif abs(u_avg - c_avg) < 2*epsilon
                %check third eigenvalue for lambda_{i+1/2}    
                Lambda_i_half = [abs(u_avg),    0,               0;...
                              0,    abs(u_avg + c_avg),      0;...
                              0,         0,        (abs(u_avg - c_avg)^2 + 4*epsilon^2) / (4*epsilon)];
%                 fprintf('3');

            elseif abs(u_mavg) < 2*epsilon
                %check first eigenvalue for lambda_{i-1/2}
                Lambda_i_mhalf = [(abs(u_mavg)^2 + 4*epsilon^2) / (4*epsilon),    0,               0;...
                                    0,                          abs(u_mavg + c_mavg),              0;...
                                    0,                          0,                   abs(u_mavg - c_mavg)];
%                 fprintf('4');

            elseif abs(u_mavg + c_mavg) < 2*epsilon
                %check second eigenvalue for lambda_{i-1/2}
                Lambda_i_mhalf = [abs(u_mavg),                 0,                                   0;...
                                    0,  (abs(u_mavg + c_mavg)^2 + 4*epsilon^2) / (4*epsilon),       0;...
                                    0,                          0,                   abs(u_mavg - c_mavg)];
%                 fprintf('5');
            ...
            elseif abs(u_mavg - c_mavg) < 2*epsilon
                Lambda_i_mhalf = [abs(u_mavg),    0,               0;...
                                0,    abs(u_mavg + c_mavg),      0;...
                                0,         0,        (abs(u_mavg - c_mavg)^2 + 4*epsilon^2) / (4*epsilon)];
%                 fprintf('6');
            end
...
title('Roe Flux-Difference Splitting Scheme with Entropy Correction','Interpreter','LaTeX');
